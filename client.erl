...
client_fun(ServerID, UserID, UserName, TweetsNumber, SubscribersNumber, ExistingUser) ->
    if
        ...
        true ->
            ...
            receive
                {acknowledgement} ->
                    io:fwrite("Registration Successful User Id: ~p User Name: ~p ~n", [UserID,UserName])
            end,
...
handle_client(UserID,UserName, TweetsNumber, SubscribersNumber, ServerID) ->
    ...
    ServerID ! {tweets, {UserID, "User" ++ UserName ++ "is a friend of User ID @" ++ MentionUser, self()}},
...
makeZipfDistribution(UserID, FollowerList, ServerID) ->
    lists:foreach(fun(N) ->
        ServerID ! {addFollower, {UserID, N, self()}} end, FollowerList).


sendTweets(Counter, UserID, UserName, ServerID) ->
    case Counter > 0 of
         true -> ServerID ! {tweets, {UserID, "User " ++ UserName ++ "tweets gibberish" ++  generate_unique_hashes(UserID, Counter)}};
         false -> done
    end,
...
retweet_handler(UserID, UserName, ServerID) ->
    ...
    receive
        {retweet_ack, {ReTweetList}} ->
            ...
            case ToRetweet ==  [] of
                true  -> done;
                false -> ServerID ! {tweets, {UserID, "User:" ++ UserName ++"Retweeting: " ++ ToRetweet , self()}}
            end
    end.
    ...
    receive
        {ackTweetSubscription, {ListOfTweets}} ->
            case ListOfTweets ==  [] of
                true  -> done;
                false -> io:fwrite("User Name: ~p Subscribed Tweets: ~n ~p ~n~n",[UserName,ListOfTweets])
            end
    end.
...
hashtag_query_handler(Hashtag, UserID, UserName, ServerID) ->
    ...
    receive
        {ackTweetHashtag,{ListOfTweets}} ->
            case ListOfTweets ==  [] of
                true  -> done;
                false -> io:fwrite("User Name: ~p  Hashtag Tweets: ~n ~p ~n~n",[UserName,ListOfTweets])
            end
    end.
...
mention_query_handler(UserID, UserName, ServerID) ->
    ...
    receive
        {ackMentionHashtag,{ListOfTweets}} ->
            case ListOfTweets ==  [] of
                true  -> done;
                false -> io:fwrite("User Name: ~p  Mentioned Tweets: ~n ~p ~n~n",[UserName,ListOfTweets])
            end
    end.
...
getAllTweets(UserID, UserName, ServerID) ->
    ...
    receive
        {ackAllTweets,{ListOfTweets}} ->
            case ListOfTweets ==  [] of
                true  -> done;
                false -> io:fwrite("User Name: ~p  All Tweets: ~n ~p ~n~n",[UserName,ListOfTweets])
            end
    end.
